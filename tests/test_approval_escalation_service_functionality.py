...
def mock_approval_center_dependencies(monkeypatch):
    ...
    return {
        ...
        "mock_st_selectbox_func": mock_st_selectbox_func # Return the specific selectbox mock
    }
...
def test_admin_sees_all_approvals_and_can_act(mock_approval_center_dependencies):
    ...
    mock_st_selectbox_func = mock_approval_center_dependencies["mock_st_selectbox_func"]

    # Simulate an admin user
    AuthService.get_user_roles.return_value = ["admin", "manager", "director", "finance"]
    AuthService.get_current_user.return_value = {"id": "admin_id", "email": "admin@example.com", "roles": ["admin", "manager", "director", "finance"]}
    RBACService.has_role.side_effect = lambda role: role in ["admin", "manager", "director", "finance"]

    # Mock Streamlit session state and functions
    mock_st_session_state.logged_in = True
    mock_st_session_state.user = {"email": "admin@example.com", "roles": ["admin", "manager", "director", "finance"]}
    mock_st_session_state.current_page = "Approval Center" # Mimic being on the approval page

    # Configure mock_st_selectbox_func with a side_effect for multiple calls
    # First call is for user selection, second is for group filter
    mock_st_selectbox_func.side_effect = [
        "admin@example.com", # Simulate selecting admin user
        "ALL"                # Simulate selecting "ALL" for approver group filter
    ]
    ...
    print(f"DEBUG (Test): mock_st_selectbox_func.side_effect configured.")
    ...
    # The first call for user selection, second for group filter
    # We configured side_effect, so we check the calls directly.
    mock_st_selectbox_func.assert_any_call("Select User", ['admin@example.com', 'manager@example.com', 'director@example.com', 'finance@example.com'])
    mock_st_selectbox_func.assert_any_call("Approver Group", ["ALL", "Manager", "Director", "Finance"])
